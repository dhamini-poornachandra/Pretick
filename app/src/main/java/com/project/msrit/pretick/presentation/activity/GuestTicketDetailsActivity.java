...
public class GuestTicketDetailsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_guest_ticket_details);

        ButterKnife.bind(this);

        ticketNo.setText(GlobalVariable.getInstance().getGuestTicketStatus()
                .get(Integer.valueOf(getIntent().getStringExtra("row"))).getTicketno());

        orgName.setText(GlobalVariable.getInstance().getGuestTicketStatus()
                .get(Integer.valueOf(getIntent().getStringExtra("row"))).getOrganization().toUpperCase());

        date.setText(GlobalVariable.getInstance().getGuestTicketStatus()
                .get(Integer.valueOf(getIntent().getStringExtra("row"))).getDate());

        startTime.setText(GlobalVariable.getInstance().getGuestTicketStatus()
                .get(Integer.valueOf(getIntent().getStringExtra("row"))).getStarttime());

        endTime.setText(GlobalVariable.getInstance().getGuestTicketStatus()
                .get(Integer.valueOf(getIntent().getStringExtra("row"))).getEndtime());

        slot.setText(GlobalVariable.getInstance().getGuestTicketStatus()
                .get(Integer.valueOf(getIntent().getStringExtra("row"))).getSlot().toUpperCase());

        contactperson.setText(GlobalVariable.getInstance().getGuestTicketStatus()
                .get(Integer.valueOf(getIntent().getStringExtra("row"))).getContactperson().toUpperCase());

        referenceApproval.setText(GlobalVariable.getInstance().getGuestTicketStatus()
                .get(Integer.valueOf(getIntent().getStringExtra("row"))).getReferenceapproval().toUpperCase());

        adminApproval.setText(GlobalVariable.getInstance().getGuestTicketStatus()
